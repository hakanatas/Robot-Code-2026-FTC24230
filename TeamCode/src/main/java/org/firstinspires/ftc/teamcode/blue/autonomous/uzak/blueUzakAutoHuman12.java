...
@Autonomous(name = "99-blue_Uzak_Human_12")
public class blueUzakAutoHuman12 extends OpMode {
    ...
    // PATH POZISYONLARI
    private final Pose startPose = new Pose(56.300, 8.800, Math.toRadians(180));
    private final Pose score1Pose = new Pose(56.300, 18.000, Math.toRadians(180));
    private final Pose humanPose = new Pose(13.000, 8.800, Math.toRadians(180));
    private final Pose score2Pose = new Pose(60.000, 26.000, Math.toRadians(180));
    private final Pose human2Pose = new Pose(13.000, 8.800, Math.toRadians(180));
    private final Pose score3Pose = new Pose(60.000, 26.000, Math.toRadians(180));
    private final Pose human3Pose = new Pose(13.000, 8.800, Math.toRadians(180));
    private final Pose score4Pose = new Pose(60.000, 26.000, Math.toRadians(180));
    private final Pose parkPose = new Pose(60.000, 35.000, Math.toRadians(180));

    // PATHLER
    private Path score1;
    private PathChain human, score2, human2, score3, human3, score4, park;
    ...
    public void buildPaths() {
        ...
        human2 = follower.pathBuilder()
                .addPath(new BezierLine(score2Pose, human2Pose))
                .setLinearHeadingInterpolation(score2Pose.getHeading(), human2Pose.getHeading())
                .build();

        score3 = follower.pathBuilder()
                .addPath(new BezierLine(human2Pose, score3Pose))
                .setLinearHeadingInterpolation(human2Pose.getHeading(), score3Pose.getHeading())
                .build();

        human3 = follower.pathBuilder()
                .addPath(new BezierLine(score3Pose, human3Pose))
                .setLinearHeadingInterpolation(score3Pose.getHeading(), human3Pose.getHeading())
                .build();

        score4 = follower.pathBuilder()
                .addPath(new BezierLine(human3Pose, score4Pose))
                .setLinearHeadingInterpolation(human3Pose.getHeading(), score4Pose.getHeading())
                .build();
    ...
    public void autonomousPathUpdate() {
        ...
        if (opmodeTimer.getElapsedTimeSeconds() >= 29.5 && pathState != 18 && pathState != -1) {
        ...
        switch (pathState) {
            ...
            case 9:
                ...
                if (pathTimer.getElapsedTimeSeconds() > HUMAN_WAIT_TIME || revolverFull) {
                    // Bekleme bitti veya revolver dolu, score3'e git
            ...
            case 12:
                ...
                if (revolverEmpty) {
                    Superstructure.stopShooting();
                    // Human3'e git + Intake AÇ
                    Superstructure.setIntakeSystem(1);
                    follower.followPath(human3);
                    setPathState(13);
                }
                break;

            case 13:
                // Human3'e gidiliyor - intake AÇIK
                Superstructure.setIntakeSystem(1);

                if (!follower.isBusy()) {
                    // Human3'e ulaştık, BEKLE!
                    setPathState(14);
                }
                break;

            case 14:
                // HUMAN3'TE BEKLEME - intake AÇIK
                Superstructure.setIntakeSystem(1);
